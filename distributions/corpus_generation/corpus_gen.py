...
def get_boundaries(num_partitions, rotation, relationships):
    out = np.arange(0,360+1,360/num_partitions) + rotation # rotation must be less than 360/divisor
    ...
    return [[out[i],out[i+1], relationships[i]] for i in np.arange(len(out) - 1)]
        ...
        for j in np.arange(angles.shape[0]):
            ...
            for boundary in boundaries:
                if (boundary[0] < angles[i,j]) and (angles[i,j] < boundary[1]):
                    vals.extend([letters[j] + ' ' + boundary[2] + ' ' + letters[i]] * int(multiplier))
                    continue
    return vals
...
def make_output(statements,
    ...
    save_statements_as_txt('../../test_models/GloVe/corpus/distance/' + 
                           distribution_name + '_' + 
                           relationship_name + '_' + 
                           'sampledas_' + sampling_distribution_name +
                           '.txt', sample)
...
if __name__ == "__main__":
    ...
    cluster_2 = np.vstack([make_cluster(int(num_points/2),1,1), make_cluster(int(num_points/2),-1,-1)])
    
    # save point distributions
    out = pd.DataFrame(shape_20)
    out.columns = ['indepV1','indepV2']
    out.to_csv('../points/shape_20.csv',index = None)
    out = pd.DataFrame(cluster_1)
    out.columns = ['indepV1','indepV2']
    out.to_csv('../points/cluster1.csv',index = None)
    out = pd.DataFrame(cluster_2)
    out.columns = ['indepV1','indepV2']
    out.to_csv('../points/cluster2.csv',index = None)
    ...
    # shape - sample as cluster1,cluster2
    ...
    make_output(statements_shape_nsew,
                ...
                dists_cluster_2,
                'cluster2')
    #cluster1 - sample as shape,cluster2
    ...
    make_output(statements_cluster_1_nsew,
                ...
                dists_cluster_2,
                'cluster2')
    #cluster2 - sample as shape,cluster1
